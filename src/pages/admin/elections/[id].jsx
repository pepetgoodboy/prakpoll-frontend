...
export default function ResultVoteAdmin() {
  const url = urlStore;
      ...
      <div className="container mx-auto px-4 py-8 font-poppins">
        ...
        >
          <div>
            ...
            <p className="capitalize">
              <strong>Program Studi:</strong> {results.studyProgram}
            </p>
            <p>
              <strong>Tanggal Mulai:</strong> {formattedDate(results.startDate)}
            </p>
            <p>
              <strong>Tanggal Selesai:</strong> {formattedDate(results.endDate)}
            </p>
          </div>
          <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-16 text-center w-11/12 mx-auto md:w-5/6">
            {results.candidateResults.map((candidate, index) => (
              ...
              >
                ...
                {/* Tampilkan daftar pemilih untuk setiap kandidat */}
                <h4 className="text-lg font-semibold mt-4">Pemilih:</h4>
                <ul className="list-disc text-left ml-6">
                  {candidate.voteBy.length > 0 ? (
                    candidate.voteBy.map((voter, i) => (
                      <li key={i}>
                        {voter.userName} - {voter.studyProgram} (waktu memilih:{" "}
                        {formattedDate(voter.voteAt)})
                      </li>
                    ))
                  ) : (
                    <li className="text-gray-500">
                      Tidak ada pemilih terdaftar untuk kandidat ini
                    </li>
                  )}
                </ul>
              </div>
